...
function runWorkerAndKill(workerPath, cycle, innerCycles, killStrategy) {
  return new Promise((resolve) => {
    ...
    child.on('message', (msg) => {
      ...
      if (killStrategy.onMessage && msg.startsWith(killStrategy.onMessage) && !killed) {
        killed = true
    ...
    async () => {
      ...
      for (let outerCycle = 0; outerCycle < TOTAL_OUTER_CYCLES; outerCycle++) {
        ...
        if (!result.killed && result.exitCode !== 0) {
          console.log(`  Worker CRASHED on its own at outer cycle ${outerCycle} (exit=${result.exitCode}): ${result.stderr.slice(0, 200)}`)
          // If the worker can't even open the DB, that's corruption — skip remaining cycles
          if (result.messages.length === 0 || !result.messages.includes('ready')) {
            console.log(`  CORRUPTION: Worker couldn't open DB at outer cycle ${outerCycle}`)
            break
          }
        }
        ...
        if (result.killed && !killStrategy.afterMs) {
          const started = result.messages.includes('ready') ||
                          result.messages.includes('schema-created') ||
                          result.messages.some(m => m.startsWith('cycle-start:'))
          expect(started).toBe(true)
        } else if (result.killed && killStrategy.afterMs && result.messages.length === 0) {
          // Worker couldn't even open before timer — WAL bloat is severe
          console.log(`  WAL BLOAT: Worker couldn't open DB within ${killStrategy.afterMs}ms at outer cycle ${outerCycle}`)
        }
          ...
          try {
            ...
            await Promise.race([
              ...
              new Promise((_, reject) =>
                setTimeout(
                  () => reject(new Error(`Intermediate open timed out at cycle ${outerCycle}`)),
                  30000,
                ),
              ),
            ])
            ...
            if (checkDb) try { await checkDb.close() } catch (_) {}
            expect.fail(`DB corrupted after ${outerCycle} kill cycles: ${err.message}`)
          }
        }
      ...
      for (const s of sizeHistory.filter((_, i) => i % 5 === 0 || i === sizeHistory.length - 1)) {
        console.log(`  Cycle ${s.outerCycle}: ${(s.sizeBytes / 1024 / 1024).toFixed(2)} MB`)
      }

      const finalSize = dirSizeBytes(dataDir)
      console.log(`Final data dir size: ${(finalSize / 1024 / 1024).toFixed(2)} MB`)
      ...
      try {
        ...
        await Promise.race([
          finalDb.waitReady,
          new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Final open timed out (WAL bloat)')), 60000),
          ),
        ])
      } catch (err) {
        console.log(`FINAL CORRUPTION after ${TOTAL_OUTER_CYCLES} cycles: ${err.message}`)
      ...
      try {
        ...
        for (const row of kindCounts.rows) {
          console.log(`  ${row.kind}: ${row.cnt} rows, avg_counter=${row.avg_counter}`)
        }
    ...
    async () => {
      ...
      for (let i = 0; i < BURST_CYCLES; i++) {
        ...
        const burstResult = await new Promise((resolve) => {
          ...
          child.on('exit', (code, sig) => {
            resolve({ killed: killed || sig === 'SIGKILL', exitCode: code, messages })
          })
        ...
        const workerCrashed = burstResult.exitCode !== 0 && burstResult.exitCode !== null
        const workerCouldntOpen = !burstResult.messages.includes('ready')

        if (workerCouldntOpen && !workerKilled) {
          console.log(`  Burst cycle ${i}: Worker couldn't open DB (corruption from previous cycles)`)
      ...
      console.log(`Burst data dir size: ${(burstSize / 1024 / 1024).toFixed(2)} MB`)
      ...
      try {
        ...
        await Promise.race([
          db.waitReady,
          new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Burst mode final open timed out')), 30000),
          ),
        ])
        ...
        if (tableCheck.rows.length > 0) {
          const count = await db.query('SELECT count(*)::int as cnt FROM wal_stress')
          console.log(`Burst mode rows: ${count.rows[0].cnt}`)

          // Full scan
          const rows = await db.query('SELECT id, cycle, kind FROM wal_stress ORDER BY id')
